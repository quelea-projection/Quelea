 ...
 public class AddBibleVerseHandler {
     ...
     public void add() {
         ...
         if (d instanceof BiblePassage) {
             ...
             StringBuilder summary = new StringBuilder(firstVerse.getChapter().getBook() + " " + passageNumber + "\n" + b.getBibleName());
             for (Bible bible : passage.getBibleVersions()) {
                 if (bible == null) {
                     return;
                 }
                 if (!summary.toString().contains(bible.getBibleName()))
                     summary.append(" + ").append(bible.getBibleName());
             }
             if (passage.getMultiLingual()) {
                 customVerses = SelectBibleVersionActionHandler.getNewVerses(passage.getBibleVersions(), new BiblePassage(summary.toString(), newVerses.toArray(new BibleVerse[i]), theme, passage.getMulti(), passage.getMultiLingual(), passage.getBibleVersions()));

             }
             replaceMap.put(passage, new BiblePassage(summary.toString(), passage.getMultiLingual() ? customVerses.toArray(new CustomVerse[0]) : newVerses.toArray(new BibleVerse[0]), theme, passage.getMulti(), passage.getMultiLingual(), passage.getBibleVersions()));


             sl.getSelectionModel()
                     .clearSelection();
             int index = -1;
             for (BiblePassage key
                     : replaceMap.keySet()) {
